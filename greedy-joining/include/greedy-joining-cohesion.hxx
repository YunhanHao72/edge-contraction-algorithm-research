...
greedy_joining_cohesion(size_t n, std::vector<std::array<size_t, 2>> edges, std::vector<VALUE_TYPE> edge_values)
{
    ...
    while (!queue.empty())
    {
        ...
        for (auto& neighbor : graph.getAdjacentVertices(merge_vertex))
        {
            ...
            if (new_weight > 0)
            {
                double reg = compute_reg_alpha(stable_vertex, neighbor.first);
                // double prio = (reg > 0) ? (double(new_weight) / reg) : INF_APPROX;
                double prio = double(new_weight) * (1 + alpha * reg);
