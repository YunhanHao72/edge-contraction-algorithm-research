...
greedy_joining_uniform(size_t n, std::vector<std::array<size_t, 2>> edges, std::vector<VALUE_TYPE> edge_values)
{
    assert(edges.size() == edge_values.size());
    constexpr double EPSILON_NOISE = 1e-9;
    constexpr double INF_APPROX = 1e20;
    constexpr double alpha = 0.3;  // cohesion strength
    constexpr double beta = 1.0;   // conflict penalty
    ...
    auto compute_reg_alpha = [&](size_t u, size_t v) -> double {
        ...
        while (it_u != nbr_u.end() && it_v != nbr_v.end()) {
            ...
            if (it_u->first == it_v->first) {
                ...
                double w_vk = it_v->second;
                if (w_uk > 0 && w_vk > 0) {
                    reg_alpha += 0.5 * (std::abs(w_uk) + std::abs(w_vk));
                }
                ++it_u;
                ++it_v;
            }
    ...
    for (size_t i = 0; i < edges.size(); ++i)
    {
        ...
        if (w > 0)
        {
            double reg_pos = compute_reg_alpha(u, v);
            double reg_neg = compute_reg(u, v);
            double prio = (reg_neg > 0)
                ? (double(w) * (1.0 + alpha * reg_pos)) / (1.0 + beta * reg_neg)
                : INF_APPROX;
    ...
    while (!queue.empty())
    {
        ...
        for (auto& neighbor : graph.getAdjacentVertices(merge_vertex))
        {
            ...
            if (new_weight > 0)
            {
                double reg_pos = compute_reg_alpha(stable_vertex, neighbor.first);
                double reg_neg = compute_reg(stable_vertex, neighbor.first);
                double prio = (reg_neg > 0)
                    ? (double(new_weight) * (1.0 + alpha * reg_pos)) / (1.0 + beta * reg_neg)
                    : INF_APPROX;
